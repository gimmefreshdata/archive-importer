...
def requestUpdate() {
  sparkRequest = '''curl --verbose --max-time <10000> -X POST http://@@HOST@@:7077/v1/submissions/create --header "Content-Type:application/json;charset=UTF-8" --data '{
...
def submissionStatus(submissionId) {
    sh([script: "curl --silent --max-time <10000> http://${getHost()}:7077/v1/submissions/status/${submissionId}", returnStdout: true])
}
